import { useGetCarByIdQuery } from "../features/Cars/carsSlice";
...
const DetailedCar = () => {
    ...
    const { data:singleCarDetails } = useGetCarByIdQuery(_id);

    console.log(singleCarDetails,"IndCar details")
    // useEffect(() => {
    //     if (cars.length === 0) {
                ...
                <div className="container mx-auto max-w-screen-xl mt-10 grid grid-cols-1 md:grid-cols-3 gap-8 px-4">
                    ...
                    <div className="md:col-span-2 flex flex-col">
                        ...
                        <div className="w-full">
                            <img
                                src={singleCarDetails.data.UserProfile}
                                alt={singleCarDetails.data.Brand}
                                className="rounded-xl w-full object-cover max-h-[400px] shadow-md"
                            />
                        </div>

                        <div className="mt-4">
                            <p className="font-bold text-[25px]">{singleCarDetails.data.Brand} {singleCarDetails.data.Model}</p>
                            <p className="text-gray-600 text-[20px]">{singleCarDetails.data.Category} · {singleCarDetails.data.Year}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                            <div className="bg-gray-100 px-4 py-6 flex flex-col items-center rounded-lg shadow-md">
                                ...
                                <p className="text-sm font-medium">{singleCarDetails.data.SeatingCapacity} Seats</p>
                            </div>
                            <div className="bg-gray-100 px-4 py-6 flex flex-col items-center rounded-lg shadow-md">
                                <img src={assets.fuel_icon} className="h-6 mb-2" alt="Fuel Type" />
                                <p className="text-sm font-medium">{singleCarDetails.data.FuelType}</p>
                            </div>
                            <div className="bg-gray-100 px-4 py-6 flex flex-col items-center rounded-lg shadow-md">
                                <img src={assets.car_icon} className="h-6 mb-2" alt="Transmission" />
                                <p className="text-sm font-medium">{singleCarDetails.data.Transmission}</p>
                            </div>
                            <div className="bg-gray-100 px-4 py-6 flex flex-col items-center rounded-lg shadow-md">
                                <img src={assets.location_icon} className="h-6 mb-2" alt="Location" />
                                <p className="text-sm font-medium">{singleCarDetails.data.Location}</p>
                            </div>
                        </div>

                        <div className="mt-6">
                            <p className="font-bold text-lg">Description</p>
                            <p className="mt-2 text-gray-600">{singleCarDetails.data.Descrption}</p>
                        </div>
                    ...
                    <div className="sticky top-24 h-fit bg-white shadow-xl p-6 rounded-lg border">
                        <div className="flex flex-row items-center justify-between">
                            <p className="text-2xl font-bold text-black-700">${singleCarDetails.data.DailyPrice}</p>
